        ...
        downloadImageBtn.addEventListener('click', function () {
            ...
            html2canvas(el, {
                ...
                try {
                    ...
                    // jsPDF UMD provides window.jspdf
                    const { jsPDF } = window.jspdf || {};
                    if (!jsPDF) {
                        // fallback: download image if jsPDF unavailable
                        const a = document.createElement('a');
                        a.href = imgData;
                        const invNoEl = document.getElementById('invoice-no');
                        const filenameImg = (invNoEl && invNoEl.value) ? `${invNoEl.value}.png` : 'invoice.png';
                        a.download = filenameImg;
                        a.click();
                        return;
                    }
                    ...
                    // use invoice number if present for filename
                    const invNoEl = document.getElementById('invoice-no');
                    const filename = (invNoEl && invNoEl.value) ? `${invNoEl.value}.pdf` : 'invoice.pdf';
                    pdf.save(filename);
                } catch (err) {
                    console.error('PDF export failed, falling back to image download', err);
                    const a = document.createElement('a');
